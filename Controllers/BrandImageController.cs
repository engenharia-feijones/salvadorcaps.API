...
namespace SalvadorCaps.API.Controllers
{
	...
	public class BrandImageController : ControllerBase
	{
		...
		private readonly ILogger<BrandImageController> _log;
		private readonly IConfiguration _config;

		public BrandImageController(ILoggerFactory loggerFactory, IConfiguration config)
		{
			_loggerFactory = loggerFactory;
			_log = loggerFactory.CreateLogger<BrandImageController>();
			_config = config;
		}
		...
		public IActionResult Post([FromBody] BrandImage image)
		{
			BrandImageBO brandImageBO;
			...
			try
			{
				...
				brandImageBO = new BrandImageBO(_loggerFactory, _config);

				image = brandImageBO.Insert(image);
		...
		[ProducesResponseType(StatusCodes.Status200OK)]
		[ProducesResponseType(StatusCodes.Status500InternalServerError)]
		public IActionResult Put(string id, [FromBody] BrandImage image)
		{
			BrandImageBO brandImageBO;
			ObjectResult response;
			...
			try
			{
				...
				image = brandImageBO.Update(image);
